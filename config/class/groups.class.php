  ...
  class group{
    ...
    public function grpAvatar($grp){
      ...
      if ($avatar == "") {
        ...
        return "{$this->DIR}/group/$grp/horizon_$avatar";
      }
    }
    ...
    public function getGrpPhotos($grp){
      ...
      if ($count == 0) {
        ...
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          <img src='{$this->DIR}/media/horizon_{$image}' alt='' data-imgby='{$universal->GETsDetails($userid, "username")}' data-postid='{$post_id}' data-time='{$Time->timeAgo($time)}' data-filter='{$filter}' class='{$filter}'></div>";
        }
      }
    }
