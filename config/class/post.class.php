  ...
  class post{
    ...
    public function imagePost($file, $value, $tags, $font, $loc, $filter, $when, $grp){
      ...
      if (in_array($ext, $allowed)) {
        if ($error == 0) {
          $new_name = time().".".$ext;
          if (move_uploaded_file($tmp_name, "../../media/horizon_".$new_name)) {
      ...
      if ($type == "text") {
        ...
        while ($trow = $tquery->fetch(PDO::FETCH_OBJ)) {
          ...
          return
            ...
            <a href='{$this->DIR}/view_post/{$post_id}'>
            <img src='{$this->DIR}/media/horizon_{$img}' alt='horizon_{$img}' class='p_img {$filter}' data-postid='{$post_id}' data-imgby='{$u}' data-time='{$Time->timeAgo($time)}' data-filter='{$filter}'> </a>";
        }
      } else if ($type == "video") {
          ...
          $text = $universal->toAbsURL($text);
           return
             "<div class='p_abt e ". self::addMoreClass($text) ."' style='font-size: {$size}px' spellcheck='false'><p>$text</p></div>
              ...
              <div class='post_marginer'></div>
              <div class='p_vid'>
              <video src='{$this->DIR}/media/horizon_{$vid}' loop preload='auto'></video>
    ...
    public function getHomePost($way, $l, $count){
      ...
      if ($query->rowCount() > 0) {
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          echo "</div><div class='p_i_1'><b>";
          echo self::addressN($address, $user_id);
          echo "</b><br><a style='color:gray;' href='". DIR ."/profile/{$universal->GETsDetails($user_id, "username")}' data-getid='{$user_id}' class='inst_username' title='{$universal->GETsDetails($user_id, "username")}'>@{$universal->nameShortener($universal->GETsDetails($user_id, "username"), 25)}</a><span title='". self::addressTitle($address, $user_id) ."'>";
          echo "</span>";
    ...
    public function getHomePostforPhotos(){
      $avatar = new Avatar;
      $universal = new universal;
      $Time = new time;
      $post_like = new postLike;
      ...
      $session = $_SESSION['id'];
        ...
        $query = $this->db->prepare("SELECT post.post_id, post.user_id, post.type, post.time, post.font_size, post.address FROM post where post.type='image' ORDER BY post.time ");
        $query->execute();
      ...
      if ($query->rowCount() > 0) {
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          echo "<div class='p_Like_wra'>";
          if($post_like->likedOrNot($post_id)){
            echo "<a class='p_unlike' data-description='Unlike'><i class='material-icons'>favorite</i></a>";
          } else if ($post_like->likedOrNot($post_id) == false) {
            echo "<span class='p_like' data-description='Like'><i class='material-icons'>favorite_border</i></span>";
          }
          echo "</div>";
    ...
    public function getPhotosPost($id){
      ...
      if ($count == 0) {
        ...
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          <img src='{$this->DIR}/media/horizon_{$image}' alt='' data-postid='{$post_id}' data-imgby='{$r}' data-time='{$Time->timeAgo($time)}' data-filter='{$filter}' class='p_pho_img {$filter}'></div>";
        }
          ...
          if ($type == "text") {
            ...
            if ($imgquery1->rowCount() > 0) {
              while ($imgrow = $imgquery1->fetch(PDO::FETCH_OBJ)) {
                ...
                if(file_exists("../../media/horizon_{$image}")){
                  unlink("../../media/horizon_$image");
                }
            ...
            if ($vidquery1->rowCount() > 0) {
              while ($vidrow = $vidquery1->fetch(PDO::FETCH_OBJ)) {
                $video = $vidrow->video;
                if(file_exists("../../media/horizon_{$video}")){
                  unlink("../../media/horizon_$video");
                }
    ...
    public function viewPost($post){
      ...
      if ($count == 0) {
        ...
        echo "</div><div class='p_cit_area'>";
        echo "<textarea class='textarea_toggle comment_og' name='f' spellcheck='true' placeholder='Wanna give your stand?'></textarea>";
        echo "<a href='#' class='comment_okay '>Save</a>";
        // echo "<span data-description='Add emojis'><i class='material-icons'>sentiment_very_satisfied</i></span>";
