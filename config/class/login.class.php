  ...
  class login_class{
    ...
    public function REGISTER($username, $firstname, $surname, $email, $password, $pri_ip){
      ...
      if (($username || $firstname || $surname || $email || $password) == "") {
        ...
        $mail->Subject = 'Verify your Horizon account';

        $mail->Body = "<span>Hello, You received this message because you created an account on Horizon.<span><br>
        <span>Click on button below to verify your Horizon account and explore.</span><br><br>
        <a href='{$url}/ajaxavi/deep/most/topmost/activate.php?id={$uid}' style='border: 1px solid #1b9be9; font-weight: 600; color: #fff; border-radius: 3px; cursor: pointer; outline: none; background: #1b9be9; padding: 4px 15px; display: inline-block; text-decoration: none;'>Activate</a>";
