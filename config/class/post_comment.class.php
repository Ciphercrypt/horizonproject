  ...
  class postComment{
    ...
    public function getComments($post){
      ...
      if ($count == 0) {
        ...
        if ($array3[0] == $session) {
          return "You gave a stand";
        } else{
          return $universal->nameShortener($universal->GETsDetails($array3[0], "username"), 15)." gave a stand";
        }

      } else if ($count == 2) {
        if (in_array($session, $array3)) {
          return "You and ".$universal->nameShortener($universal->GETsDetails($array3[0], "username"), 15)." gave a stand";
        } else {
          return $universal->nameShortener($universal->GETsDetails($array3[0], "username"), 15)." and ".$universal->nameShortener($universal->GETsDetails($array3[1], "username"), 15)." gave a stand";
        }

      } else if ($count == 3) {
        if (in_array($session, $array3)) {
          return "You, ".$universal->nameShortener($universal->GETsDetails($array3[0], "username"), 15)." and 1 other gave a stand";
        } else {
          return $universal->nameShortener($universal->GETsDetails($array3[0], "username"), 15).", ".$universal->nameShortener($universal->GETsDetails($array3[1], "username"), 15)." and 1 other gave a stand";
        }

      } else if ($count > 3) {
        $slice = array_slice($array3, 2);
        if (in_array($session, $array3)) {
          return "You, ".$universal->nameShortener($universal->GETsDetails($array3[0], "username"), 15)." and ". count($slice) ." others gave a stand";
        } else {
          return $universal->nameShortener($universal->GETsDetails($array3[0], "username"), 15).", ".$universal->nameShortener($universal->GETsDetails($array3[1], "username"), 15)." and ". count($slice) ." others gave a stand";
        }
      }

      // return " gave a stand (".self::simpleGetComments($post).")";
    ...
    public function imageComment($file, $post){
      ...
      if($settings->AmIBlocked($to) == false){
        if (in_array($ext, $allowed)) {
          if ($error == 0) {
            $new_name = time().".".$ext;
            if (move_uploaded_file($tmp_name, "../../comments/horizon_".$new_name)) {
      ...
      if ($count > 0) {
        while ($row = $query->fetch(PDO::FETCH_OBJ)) {
          ...
          if ($type == "text") {
            ...
            $return = "<img src='{$this->DIR}/comments/horizon_{$data}' class='comments_img' data-imgby='{$z}' data-time='{$Time->timeAgo($time)}'>";

          } else if ($type == "sticker") {
            $return = "<img src='{$this->DIR}/comments/horizon_{$data}' class='comment_sticker'>";
          }
          ...
          echo "' data-commentid='{$comment_id}'><img class='comments_avatar' src='{$this->DIR}/{$avatar->GETsAvatar($user_id)}'>
            ...
            if ($type == "text") {
              ...
              echo "</div><a class='comment_likes' href='#' title='Likes'>". self::comment_likes($comment_id) ."</a>
                <span class='comments_time' title='{$Time->normalTime($time)}'>{$Time->timeAgo($time)} </span>
              </div>
            </div>";
            if (self::MyCommentOrNot($comment_id)) {
